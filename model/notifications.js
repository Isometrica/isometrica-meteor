...
Meteor.methods({
	 ...
	 * @author Mark Leusink
   * 
   * @param toId    string/ list of strings containing the ids of the users to send the notification to
	 *
	 */
    "sendToInbox" : MultiTenancy.method( function(toId, subject, contents) { 
      ...
      for (var i=0; i<toId.length; i++) {
        _notifyHelpers.notify(toId[i], subject, contents);
      }
...
_notifyHelpers = {
  ...
  notify : function(toId, subject, contents) {
